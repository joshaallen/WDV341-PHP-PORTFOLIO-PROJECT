            ...
            <!--end of coantainer-->
                ...
                <?php 
                    ...
                    try{
                        ...
                        $posts ="";
                        $sql = "SELECT COUNT(1) FROM posts ORDER BY post_id DESC";
                        if ($res = $dbObject->connect()->query($sql)) {
                            if($res->fetchColumn()>0) {
                                $stmt = $dbObject->connect()->prepare("SELECT * FROM posts INNER JOIN posts_user ON posts_user.posts_user_id=posts.post_author ORDER BY post_id DESC");
                                $stmt->execute();
